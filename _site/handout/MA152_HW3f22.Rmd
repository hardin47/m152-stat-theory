...
Suppose that $X_1, \ldots , X_n$ form a random sample from the exponential distribution with parameter $\theta$. 
...
Suppose that the time in minutes required to serve a customer at a certain facility has an exponential distribution for which the value of the parameter $\theta$ is unknown, the prior distribution of $\theta$ is a gamma distribution for which the mean is 0.2 and the standard deviation is 1, and the average time required to serve a random sample of 20 customers is observed to be 3.8 minutes. 
